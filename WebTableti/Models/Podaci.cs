    ...
    public class Podaci
    {
        ...
        public DataTable NapuniGrid()
        {
            ...
            using (conn = new SqlConnection(connString))
            {
                ...
                conn.Open();
                cmd = new SqlCommand("Select * from NF_Tracc_fermi_Grupa3 where Email=@email and LastStopCode in (50002,50007,50008) order by Email, DateRec desc", conn);
        ...
        public DataTable NapuniGrupu3()
        {
            ...
            {
                conn.Open();
                cmd = new SqlCommand("select * from NF_Tracc_fermi_Grupa3 where StopCode in (50002,50007,50008)", conn);
                SqlDataAdapter adapter = new SqlDataAdapter(cmd);
                adapter.Fill(dtPodaci);
            }          
